...
pub async fn worldcup_scoreboard() -> Redirect {
    dayless_scoreboard(RouteLeague::WorldCup)
}

pub async fn worldcup_scoreboard_today(
    State(state): State<AppState>,
) -> Result<Response, AppError> {
    date_scoreboard_today(&state, RouteLeague::WorldCup).await
}

pub async fn worldcup_scoreboard_day(
    State(state): State<AppState>,
    Path(day): Path<String>,
) -> Result<Html<String>, AppError> {
    date_scoreboard_day(&state, RouteLeague::WorldCup, &day).await
}

pub async fn worldcup_game(
    State(state): State<AppState>,
    Path((day, game_id)): Path<(String, String)>,
) -> Result<Html<String>, AppError> {
    date_game(&state, RouteLeague::WorldCup, &day, &game_id).await
}

pub async fn worldcup_standings(State(state): State<AppState>) -> Result<Html<String>, AppError> {
    let standings = state.data.worldcup_standings().await?;
...
async fn date_game(
    ...
    let html = match league {
        ...
        RouteLeague::WorldCup => {
            let game = state.data.worldcup_game(&game_id).await?;
            render::soccer_scoreboard_page_with_today(
                parsed_day,
                &scoreboard,
                game.as_ref(),
                today_day,
